#ifndef __HASH_MULTIMAP_H__
#define __HASH_MULTIMAP_H__
...
namespace ch17 {
    ...
    template<class Key, class T, class H = Hash<Key>,
        class EQ = std::equal_to<Key>, class A = allocator<pair<Key, T> > >
    class hash_multimap
    {
    public:
    ...
    public:
        class hash_iterator
        {
            ...
            b_iterator i2;
        public:
            typedef hash_multimap::value_type value_type;
            typedef hash_multimap::difference_type difference_type;
            typedef hash_multimap::pointer pointer;
            typedef hash_multimap::reference reference;
            typedef std::bidirectional_iterator_tag iterator_category;

            hash_iterator() {}
            hash_iterator(hash_multimap& map, v_iterator i1, b_iterator i2) : map(map), i1(i1), i2(i2) {}
        ...
        hash_multimap(size_type n = 101, const H& h = H(), const EQ& e = EQ()) :
        ...
        template<class In> hash_multimap(In first, In last,
        ...
        void swap(hash_multimap& x) { b.swap(x.b); v.swap(x.v); std::swap(sz,x.sz); }
        hash_multimap& operator=(const hash_multimap&);
    };

    template<class Key, class T, class H, class EQ, class A>
    typename hash_multimap<Key,T,H,EQ,A>::iterator hash_multimap<Key,T,H,EQ,A>::find(const key_type& k)
    {
    ...
    template<class Key, class T, class H, class EQ, class A>
    void hash_multimap<Key,T,H,EQ,A>::resize(size_type s)
    {
        size_type i = v.size();
        if(s <= b.size()) return;
        while(i >= 0) {
    ...
    template<class Key, class T, class H, class EQ, class A>
    std::pair<typename hash_multimap<Key,T,H,EQ,A>::iterator, bool> hash_multimap<Key,T,H,EQ,A>::insert(const value_type& val)
    {
        size_type i = hash(val.first)%b.size();
        if(b[i]) {
            Entry* q;
            for(Entry* p = b[i]; p; p = p->next)
            {
            ...
            v.push_back(Entry(val.first,0,v.size()));
            q->next = &v.back();
