...
// this is the same as list from ex16, but I added subscript and at() operations the
// simplest way possible. no funny tricks are used.
//
// obviously in a linked list, subscripting is going to cost O(n) time, with elements
// toward the end costing more than elements closer to the beginning - and the larger
...
int main() {
	...
	try {
		...
		std::cout << "std::out_of_range caught, as expected" << std::endl;
	}
}
