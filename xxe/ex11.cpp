...
namespace xxe {
    ...
    // This was actually much simpler, however,it ALWAYS copies all elements, so it will be significantly slower
    // for most inserts.
    template<class T, class A>
    void vector<T,A>::insert(T* p, size_type n, const T& x) {
        vector_base<T,A> b(this->alloc, size() + n);
    ...
    // Also simpler, but it always copies all elements as well.
    template<class T, class A>
    T* vector<T,A>::erase(T* first, T* last) {
        ...
        if(first > this->v) {
            std::uninitialized_copy(this->v, first, b.v);
            b.space += first - this->v;
        }
        if(last < this->space) {
            std::uninitialized_copy(last, this->space, b.space);
            b.space += (this->space - last);
        }
        destroy_elements();
        xxe::swap(b, *this);
        return b.v + (first - this->v);
    }
}
