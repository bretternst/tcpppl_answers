...
// I believe this was mostly safe already by virtue of using only vectors for its backing store.
// The main dangerous operation was resize() - so I made this safer (but less performant) by
// copying the backing vectors, modifying them, and swapping them back in.
namespace xxe {
    using std::allocator;
    using std::pair;
    ...
    void hash_map<Key,T,H,EQ,A>::resize(size_type s)
    {
        ...
        std::vector<Entry,typename A::template rebind<Entry>::other> vnew(v);
        std::vector<Entry*,typename A::template rebind<Entry*>::other> bnew;
        bnew.resize(s);

        std::fill(bnew.begin(),bnew.end(),static_cast<Entry*>(0));
        vnew.reserve(static_cast<unsigned int>(s*max_load));
        for(size_type i = 0; i<vnew.size(); i++)
        {
            size_type ii = hash(vnew[i].p.first)%bnew.size();
            vnew[i].next = bnew[ii];
            vnew[i].pos = i;
            bnew[ii] = &vnew[i];
        }
